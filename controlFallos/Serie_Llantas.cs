...
namespace controlFallos
{
    public partial class Serie_Llantas : Form
    {
        ...
        public Serie_Llantas(validaciones v, int idUsuario, int empresa, int area,double cantidad)
        {
            this.v = v;
        ...
        private void AgregarSeries(object sender, EventArgs e)
        {
            if (Contador <= cantidad)
            {
                ...
                {
                    v.c.insertar("INSERT INTO cseries_llantas(serie, FechaHora) values ('" + txtSeries.Text + "', now())");
                    Contador++;
                    Limpiar();
                }
                else
