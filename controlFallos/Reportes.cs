    ...
    public partial class Reportes : Form
    {
       ...
       /* public DataSet datosG(string sql, int valor)
        ...
        public DataSet cargarGridSC(string Busqueda)
        {
            ...
            adaptador = v.getReport("SET lc_time_names = 'es_ES';Select  convert(concat(t1.consecutivo, '-', t1.descripcioneco),char) as Economico, convert(t3.codrefaccion,char) as Codigo, convert(t3.nombreRefaccion,char) as Nombre, convert(t2.cantidad,char) as Cantidad, convert(t3.CostoUni,char) as 'Costo Compra', convert(t5.Simbolo, char) as 'Moneda', ROUND(((t3.CostoUni * (select Costo from ctipocambio where idtipoCambio = t3.tipoMonedafkCTipoCambio)) * (5 /100)) + (t3.CostoUni * (select Costo from ctipocambio where idtipoCambio = t3.tipoMonedafkCTipoCambio)),2) as 'Costo Venta', ROUND(((t3.CostoUni * (select Costo from ctipocambio where idtipoCambio = t3.tipoMonedafkCTipoCambio)) * (5 /100) + (t3.CostoUni * (select Costo from ctipocambio where idtipoCambio = t3.tipoMonedafkCTipoCambio)))* t2.Cantidad,2)as 'Total',upper(date_format(convert(t2.FechaHora, char),'%d/%M/%Y %H:%i %p')) as Salida, if(t3.Tipo = 1, 'NUEVA', if(t3.Tipo = 2, 'REMANOFACTURADA', if(t3.Tipo = 3, 'PODUCCION',''))) as 'TIPO'   from cunidades as t1 inner join ccarrocero as t2 on t1.idunidad = t2.unidadfkCUnidades inner join crefacciones as t3 on t3.idrefaccion = t2.refaccionfkCRefacciones inner join datosistema as t4 on t4.usuariofkcpersonal = t2.usuariofkCPersonal inner join ctipocambio as t5 on t3.tipoMonedafkCTipoCambio = t5.idtipoCambio" + Busqueda);
            dt.Dispose();
            dt.EndInit();
            adaptador.Fill(ds);
            Ds.Merge(ds);
