...
namespace controlFallos
{
    public partial class percances : Form
    {
        ...
        void initializeSupervisors() { v.iniCombos("SELECT idPersona as id, UPPER(CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,''))) as nombre FROM cpersonal WHERE status=1 AND empresa = 1 AND area = 1 ORDER BY CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,'')) ASC", cbxgetEXTsupervisor, "id", "nombre", "-- SELECCIONE SUPERVISOR --"); }
                ...
                if (ews == DialogResult.OK)
                {
                    if (filename.Trim() != "")
                    {
                        ...
                        if (res[39] == "1")
                        {
                            ...
                            img2 = v.ImageToString(ediciionBitmap(Properties.Resources.LATERALDERECHO, imgtemp[1]));
                            img3 = v.ImageToString(ediciionBitmap(Properties.Resources.LATERALIZQUIERDO, imgtemp[2]));
        ...
        private void btnHDSupervisor_Click(object sender, EventArgs e)
        {
            if (cbxgetEXTsupervisor.SelectedIndex > 0 || supervisorAsistenciaPercanceAnterior > 0)
            {
                ...
                if (res == DialogResult.OK)
                {
                    ...
                    lblsupervisor.Text = v.getaData("SELECT CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,'')) FROM cpersonal WHERE idpersona ='" + supervisorFinalizar + "'").ToString();
                }
            }
            else
        ...
        private void cbxgeteco_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbxgeteco.SelectedIndex > 0 && cbxgeteco.DataSource != null)
            {
                ...
                if (Convert.ToInt32(modelo) == 1)
                {
                    ...
                    panel2.BackgroundImage = controlFallos.Properties.Resources.LATERALDERECHO;
                    panel3.BackgroundImage = controlFallos.Properties.Resources.LATERALIZQUIERDO;
        ...
        private void btnHDConductor_Click(object sender, EventArgs e)
        {
            if (idconductorAnterior > 0 || idconductor > 0)
            {
                ...
                if (res == DialogResult.OK)
                {
                    ...
                    lblconductor.Text = v.getaData("SELECT CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,'')) FROM cpersonal WHERE idpersona ='" + conductorFinalizar + "'").ToString();
                }
            }
            else
        ...
        private void txtgetcredencial_TextChanged(object sender, EventArgs e)
        {
            ...
            object res = v.getaData(string.Format("SELECT CONCAT(idpersona,';',coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,'')) FROM cpersonal WHERE credencial ='{0}' AND empresa='1' AND area='1'", txtgetcredencial.Text.Trim()));
            if (res != null)
            {
        ...
        void initializeConductorsBusq() { v.iniCombos("SELECT DISTINCT t2.idpersona as id, UPPER(CONCAT(coalesce(t2.nombres,''),' ',coalesce(t2.apPaterno,''), ' ', coalesce(t2.apMaterno,''))) as nombres FROM reportepercance as t1 INNER JOIN cpersonal as t2 ON t1.conductorfkcpersonal = t2.idpersona GROUP BY conductorfkcpersonal ORDER BY t2.nombres ASC", cbxgetconductor, "id", "nombres", "--SELECCIONE UN CONDUCTOR--"); }
        ...
        public void initializeReports(string where)
        {
            ...
            DataTable dt = (DataTable)v.getData("SET lc_time_names='es_ES';SELECT t1.idreportePercance,concat('RP-',LPAD(t1.consecutivo,7,0)),concat(t3.identificador,LPAD(t2.consecutivo,4,'0')),UPPER((SELECT CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,'')) FROM cpersonal WHERE idpersona=t1.conductorfkcpersonal)),UPPER(COALESCE(date_format(t1.fechaHoraAccidente,'%d/%M/%Y %H:%i'),'')), COALESCE((SELECT UPPER(concat(nombre,' - ',Descripcion)) FROM cservicios WHERE t1.servicioenlaborfkcservicios=idservicio),''),COALESCE(UPPER(t1.lugaraccidente),''),if(direccion is not null,UPPER(if(direccion=1,'Norte','Sur')),''),UPPER(if(finalizado = 1,'Finalizado','En Proceso')),coalesce(UPPER(t1.sintesisocurrido),''),coalesce((SELECT UPPER(CONCAT(coalesce(nombres,''),' ',coalesce(apPaterno,''),' ',coalesce(apMaterno,''))) FROM cpersonal WHERE idpersona=t1.supervisorkcpersonal),'') FROM reportepercance as t1 INNER JOIN cunidades as t2 ON t1.ecofkcunidades=t2.idunidad INNER JOIN careas as t3 ON t2.areafkcareas=t3.idarea " + where + " order by t1.consecutivo DESC;");
